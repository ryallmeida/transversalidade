...
#LDO 2026.pdf  2469  15255       260
#Relatorio_Manutencao_Patrimonio_Publico_2020.pdf   373   1199         1
#Relatório_Projetos_em_andamento_2019.pdf  1066   2265        15

#na versão anterior ainda não havia-se disponibilizado o ldo do ano de 2026, nessas correções estamos considerando

library(quanteda)
library(stringr)

# extrair o ano diretamente do nome do arquivo
anos_ldo <- str_extract(basename(arquivos_ldo), "\\d{4}") |> as.numeric()

df_docs <- data.frame(
  doc_id = basename(arquivos_ldo),
  ano    = anos_ldo,
  texto  = textos_unificados_ldo,
  stringsAsFactors = FALSE
)


lista_parag <- lapply(1:nrow(df_docs), function(i) {
  
  texto <- df_docs$texto[i]
  
  parag <- unlist(strsplit(
    texto,
    split = "\n\\s*\n",
    perl = TRUE
  ))
...
for (i in seq_along(df_corpus_ldo$text)) {
  caminho <- file.path("C:/Users/ryall/Documents/transversalidade/LDO/LDO_txt",paste0(tools::file_path_sans_ext(df_corpus_ldo$doc_id[i]), ".txt"))
  writeLines(df_corpus_ldo$text[i], con = caminho)
}

# Lê todos os .txt como um corpus novamente
textos_ldo_readtext <- readtext("C:/Users/ryall/Documents/transversalidade/LDO/LDO_txt/*.txt", text_field = "text")

# Transforma em corpus do quanteda
corpus_ldo2 <- corpus(textos_ldo_readtext)

tokens_ldo <- quanteda::corpus_subset(corpus_ldo2) |>
  tokens(
    what = "word",
    remove_punct = TRUE,
    remove_symbols = TRUE,
